(function (root) {
  ...
  function init() {
    ...
    document.querySelectorAll("[data-sample-url]").forEach(b => b.onclick = () => { const key=b.dataset.sampleUrl; const value=key === "science" ? activities[0].url : `https://mock.example.test/${key}`; document.querySelector("#event-url").value=value; assess(value); });
